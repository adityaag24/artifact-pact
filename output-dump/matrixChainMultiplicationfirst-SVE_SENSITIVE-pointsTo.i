...
ptsTo(readyWorkList) := [nullCell];
ptsTo(leftPointer) := [nullCell];
ptsTo(rightPointer) := [nullCell];
ptsTo(P) := [heapCell#0];
ptsTo(M) := [heapCell#2];
...
void matrix_paranth_for_len(int i, int l) {
    ...
    if (l <= N) {
        ...
        matrix_paranth_for_len(i, _imopVarPre111);
        /*OUT: [ptsTo(stdin) := [nullCell];
        ptsTo(stdout) := [nullCell];
        ptsTo(stderr) := [nullCell];
        ptsTo(readyWorkList) := [nullCell];
...
int main(int argc, char **argv) {
    ...
    ptsTo(fp) := [nullCell];
    ptsTo(_imopVarPre122) := [N];
    ptsTo(_imopVarPre127) := [heapCell#0];
    ptsTo(argv) := [heapCell#4];
    ]*/
    /*OUT: [ptsTo(stdin) := [nullCell];
    ptsTo(stdout) := [nullCell];
    ...
    for (i = 0; i <= N; i++) {
        ...
        ptsTo(_imopVarPre127) := [heapCell#0];
        ptsTo(argv) := [heapCell#4];
        ptsTo(_imopVarPre129) := [heapCell#0];
        ]*/
        _imopVarPre129 = &P[i];
        /*OUT: [ptsTo(stdin) := [nullCell];
        ptsTo(stdout) := [nullCell];
    ...
    ptsTo(_imopVarPre122) := [N];
    ptsTo(_imopVarPre127) := [heapCell#0];
    ptsTo(_imopVarPre135) := [heapCell#2];
    ptsTo(argv) := [heapCell#4];
    ]*/
    /*OUT: [ptsTo(stdin) := [nullCell];
    ptsTo(stdout) := [nullCell];
    ...
    for (i = 0; i <= N; i++) {
        ...
        ptsTo(_imopVarPre135) := [heapCell#2];
        ptsTo(argv) := [heapCell#4];
        ptsTo(_imopVarPre141) := [heapCell#1];
        ptsTo(heapCell#2) := [heapCell#1];
        ]*/
        /*OUT: [ptsTo(stdin) := [nullCell];
        ptsTo(stdout) := [nullCell];
