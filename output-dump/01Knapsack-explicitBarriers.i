...
void knapSack(int i, int k) {
    if (i < N) {
        ...
        if (k < weights[i]) {
            ...
            int _imopVarPre140;
            int _imopVarPre141;
            _imopVarPre140 = (oldProfit[k]) > (profits[i] + oldProfit[k - weights[i]]);
            if (_imopVarPre140) {
                _imopVarPre141 = (oldProfit[k]);
            } else {
                _imopVarPre141 = (profits[i] + oldProfit[k - weights[i]]);
            }
            newProfit[k] = _imopVarPre141;
        }
        {
            int wxyz = 0;
            myWaitAll(1);
