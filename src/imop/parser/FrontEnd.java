...
public class FrontEnd {
	...
	public static <T extends Node> T parseCrude(InputStream input, Class<T> nodeType) {
		if (nodeType == TranslationUnit.class) {
			...
			CParser.types.put("_Float128", Boolean.TRUE);
...
//			}
	...
	public static CompoundStatement simplifyExpression(String expression) {
		CompoundStatement node = null;
		try {
			node = FrontEnd.parseAndNormalize(expression,CompoundStatement.class);	
		}catch(Exception e) {
			node = FrontEnd.parseAndNormalize("{\n" + expression + "\n}",CompoundStatement.class);
		}
		
		Set<CompoundStatement> originalCS = Misc.getInheritedEnclosee(node, CompoundStatement.class);
		SimplificationString ret = node.accept(new ExpressionSimplifier(originalCS));
		node = FrontEnd.parseAndNormalize(ret.getReplacementString().toString(), CompoundStatement.class);
		return node;
	}
