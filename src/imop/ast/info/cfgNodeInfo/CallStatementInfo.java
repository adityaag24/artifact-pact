...
//					System.err.println("Cannot find any parent for " + callStmt.getFunctionDesignatorNode());
	...
	public void setCalledDefinitions(List<FunctionDefinition> functionDefinitions) {
		this.calledDefinitions = functionDefinitions;
	}
		...
		if (this.calledDefinitions == null) {
			...
			if (this.isACallByFunctionPointer()) {
				...
				if (funcDesig != null) {
					...
					if(set.isUniversal()) {
						for(FunctionDefinition functionDefinition: Program.getRoot().getInfo().getAllFunctionDefinitions()) {
							if(functionDefinition == Program.getRoot().getInfo().getMainFunction()) {
								continue;
							}
							calledDefinitions.add(functionDefinition);
						}
					} else {
