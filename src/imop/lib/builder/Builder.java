...
public class Builder {
	...
	public static String getNewTempName(String buffer) {
		if (Program.isPrePassPhase) {
			return buffer + "_ivp" + counterVar++;
		} else {
			return buffer + "_iv" + counterVar++;
		}
	}

	public static String myGetNewTempName(String buffer) {
		if (Program.isPrePassPhase) {
	...
	public static void addFunctionToGlobal(int index, FunctionDefinition funcDef) {
		...
		/*
		 * NEW CODE:
		 * Add this function definition to the global symbol table.
		 */

		String functionName = funcDef.getInfo().getFunctionName();
		Type functionType = Type.getTypeTree(funcDef, Program.getRoot());
		Program.getRoot().getInfo().getSymbolTable().put(functionName,
				new Symbol(functionName, functionType, funcDef,Program.getRoot()));
	}
