...
public class StringGetter {
	...
	public static String myGetRenamedString(Node node, HashMap<String, String> renamingMap) {
		MyRenamingStringGetter stringGetter = new MyRenamingStringGetter(renamingMap);
		node.accept(stringGetter);
		return stringGetter.str.toString().replaceAll("\n\\s*\n", "\n");
	}
	 ...
	 * {@code changeSource} replaced by the string {@code replacementString} (if the
	 * replacement would actually be possible in the AST). <br>
	 * Note that no resulting changes are done to the AST of {@code baseNode} or
	 * {@code changeSource}. Also note that this method is very different from a
	 * simple substring replacement.
	 * 
	 * @param baseNode          node whose string equivalent is to be found, with
	 ...
	 * @return string of {@code baseNode}, with the string of {@code changeSource},
	...
	private static class RenamingStringGetter extends InternalStringGetter {
		...
		public void visit(FunctionDefinition n) {
			...
			/*
			 * Now, remove all those mappings that may get redefined in the symbol table of
			 * this scope, before entering the scope.
			 */
		...
		/**
		 * f0 ::= <INITIALIZER> f1 ::= "(" f2 ::= <IDENTIFIER> f3 ::= "=" f4 ::=
		 * Initializer() f5 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <IDENTIFIER> f1 ::= "=" f2 ::= <IDENTIFIER> f3 ::= "+" f4 ::=
		 * AdditiveExpression()
		 */
		@Override
		...
		/**
		 * f0 ::= <IDENTIFIER> f1 ::= "=" f2 ::= MultiplicativeExpression() f3 ::= "+"
		 * f4 ::= <IDENTIFIER>
		 */
		@Override
	...
	private static class MyRenamingStringGetter extends InternalStringGetter {
		HashMap<String, String> renamingMap;

		public MyRenamingStringGetter(HashMap<String, String> renamingMap) {
			this.renamingMap = renamingMap;
		}
		...
		public void visit(CallStatement n) {
			printCommentorsAndPragmas(n.getPreCallNode());
			// Step 1a: Add declaration for each identifier being used in the LHS and RHS of
			// the CallStatement.
			Set<SimplePrimaryExpression> setOfSPE = new HashSet<>();
			if (n.getPostCallNode().hasReturnReceiver()) {
				setOfSPE.add(n.getPostCallNode().getReturnReceiver());
			}
			setOfSPE.addAll(n.getPreCallNode().getArgumentList());
			str.append("{");
			// Step 1b: Initialize and populate renamingMapPrime (with x:x')
			renamingMapPrime = new HashMap<>();
			for (SimplePrimaryExpression spe : setOfSPE) {
				if (spe.isAnIdentifier()) {
					NodeToken idToken = spe.getIdentifier();
					String id = idToken.getTokenImage();
					String newName = Builder.getNewTempName(id);
					if(renamingMap.containsKey(id)) {
						renamingMapPrime.put(id, newName);
						Symbol symbol = Misc.getSymbolEntry(id, n);
						if(symbol == null){
							System.out.println("Identifier : "+id);							
						}
						Declaration meow = Misc.getSymbolEntry(id, n).getType().getDeclaration(newName);
						str.append(meow);							
			...
			// Step 2: Add assignments of the form x' = env->x;
			for (String x : renamingMapPrime.keySet()) {
				str.append(renamingMapPrime.get(x) + "=" + renamingMap.get(x) + ";\n");
			}
			printLabels(n);
			CallStatement node = n;
			if (node.getPostCallNode().hasReturnReceiver()) {
		...
		Map<String, String> renamingMapPrime = null;
		...
		public void visit(SimplePrimaryExpression n) {
			if (n.isAConstant()) {
				...
				NodeToken idToken = n.getIdentifier();
				String id = idToken.getTokenImage();
				if (renamingMapPrime.containsKey(id)) {
					str.append(renamingMapPrime.get(id));
				} else {
					str.append(id);
				}
			}
	...
	private static class InternalStringGetter extends DepthFirstProcess {
		...
		/**
		 * f0 ::= Declaration() | FunctionDefinition() | DeclareReductionDirective() |
		...
		/**
		 * f0 ::= <RESTRICT> | <CONST> | <VOLATILE> | <INLINE> | <CCONST> | <CINLINED> |
		 * <CINLINED2> | <CSIGNED> | <CSIGNED2>
		 */
		@Override
		...
		/**
		 * f0 ::= StructOrUnion() f1 ::= ( <IDENTIFIER> )? f2 ::= "{" f3 ::=
		 * StructDeclarationList() f4 ::= "}"
		 */
		@Override
		...
		/**
		 * f0 ::= StructOrUnion() f1 ::= <IDENTIFIER>
		 */
		@Override
		...
		/**
		 * f0 ::= <ENUM> f1 ::= ( <IDENTIFIER> )? f2 ::= "{" f3 ::= EnumeratorList() f4
		 * ::= "}"
		 */
		@Override
		...
		/**
		 * f0 ::= <IDENTIFIER> f1 ::= ( "=" ConstantExpression() )?
		 */
		@Override
		...
		/**
		 * f0 ::= AbstractDeclaratorClosed() | DimensionSize() |
		...
		/**
		 * f0 ::= ParallelConstruct() | ForConstruct() | SectionsConstruct() |
		 * SingleConstruct() | ParallelForConstruct() | ParallelSectionsConstruct() |
		 * TaskConstruct() | MasterConstruct() | CriticalConstruct() | AtomicConstruct()
		 * | OrderedConstruct()
		 */
		@Override
		...
		/**
		 * f0 ::= BarrierDirective() | TaskwaitDirective() | TaskyieldDirective() |
		 * FlushDirective()
		 */
		@Override
		...
		/**
		 * f0 ::= <PARALLEL> f1 ::= UniqueParallelOrDataClauseList() f2 ::= OmpEol()
		 */
		@Override
		...
		/**
		 * f0 ::= OmpPrivateClause() | OmpFirstPrivateClause() | OmpLastPrivateClause()
		 * | OmpSharedClause() | OmpCopyinClause() | OmpDfltSharedClause() |
		 * OmpDfltNoneClause() | OmpReductionClause()
		 */
		@Override
		...
		/**
		 * f0 ::= <PRIVATE> f1 ::= "(" f2 ::= VariableList() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <FIRSTPRIVATE> f1 ::= "(" f2 ::= VariableList() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <LASTPRIVATE> f1 ::= "(" f2 ::= VariableList() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <SHARED> f1 ::= "(" f2 ::= VariableList() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <COPYIN> f1 ::= "(" f2 ::= VariableList() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <REDUCTION> f1 ::= "(" f2 ::= ReductionOp() f3 ::= ":" f4 ::=
		 * VariableList() f5 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <FOR> f1 ::= UniqueForOrDataOrNowaitClauseList() f2 ::= OmpEol()
		 */
		@Override
		...
		/**
		 * f0 ::= <ORDERED> | UniqueForClauseSchedule() | UniqueForCollapse()
		 */
		@Override
		...
		/**
		 * f0 ::= <COLLAPSE> f1 ::= "(" f2 ::= Expression() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <SCHEDULE> f1 ::= "(" f2 ::= ScheduleKind() f3 ::= ( "," Expression()
		 * )? f4 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= <STATIC> | <DYNAMIC> | <GUIDED> | <RUNTIME>
		 */
		@Override
		...
		/**
		 * f0 ::= <FOR> f1 ::= "(" f2 ::= OmpForInitExpression() f3 ::= ";" f4 ::=
		 * OmpForCondition() f5 ::= ";" f6 ::= OmpForReinitExpression() f7 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= OmpPragma() f1 ::= <SECTION> f2 ::= OmpEol() f3 ::= Statement()
		 */
		@Override
		...
		/**
		 * f0 ::= <COPYPRIVATE> f1 ::= "(" f2 ::= VariableList() f3 ::= ")"
		 */
		@Override
		...
		/**
		 * f0 ::= OmpPragma() f1 ::= <THREADPRIVATE> f2 ::= "(" f3 ::= VariableList() f4
		 * ::= ")" f5 ::= OmpEol()
		 */
		@Override
		...
		/**
		 * f0 ::= OmpPragma() f1 ::= <DECLARE> f2 ::= <REDUCTION> f3 ::= "(" f4 ::=
		 * ReductionOp() f5 ::= ":" f6 ::= ReductionTypeList() f7 ::= ":" f8 ::=
		 * Expression() f9 ::= ")" f10 ::= ( InitializerClause() )? f11 ::= OmpEol()
		 */
		@Override
		...
		/**
		 * f0 ::= SimpleLabeledStatement() | CaseLabeledStatement() |
		...
		/**
		 * f0 ::= <CASE> f1 ::= ConstantExpression() f2 ::= ":" f3 ::= Statement()
		 */
		@Override
		...
		/**
		 * f0 ::= GotoStatement() | ContinueStatement() | BreakStatement() |
		 * ReturnStatement()
		 */
		@Override
		...
		/**
		 * f0 ::= LogicalORExpression() f1 ::= ( "?" Expression() ":"
