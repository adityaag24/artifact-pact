...
/**
 * Returns the type of the expression being visited. If any sub-expression has
 * unknown type, then this method should gracefully return {@code null}, and not
 * throw any exception. TODO: Handle type qualifiers as well.
 */
public class ExpressionTypeGetter extends GJNoArguDepthFirstProcess<Type> {

	private Scopeable pointOfInsertion;

	public ExpressionTypeGetter() {
		this.pointOfInsertion = null;
	}

	public ExpressionTypeGetter(Scopeable pointOfInsertion) {
		this.pointOfInsertion = pointOfInsertion;
	}

	/**
	 * This method performs pointer generation on {@code inputType}, as per the
	 * semantics of the C language. Note that the pointer-generation is performed
	 * for types of all the expressions and subexpressions, except when they are an
	 * operand of the unary {@code &}, {@code ++}, {@code --}, {@code sizeof}, or
	 * LHS operand of {@code =} or {@code .} operators. However, unless we encounter
	 * these operators, we don't assume that these operators exist on the operand
	 * under concern. Hence, this method <i>always</i> performs the pointer
	 * generation on all array and function types.
	 * 
	 * @param inputType type on which pointer generation has to be performed.
	 * @return the pointer-generated type, regardless of where the corresponding
	 *         operand may exist.
	 ...
	 * performed. This method is useful to undo the effects of pointer generation,
	 * at the following locations: the unary {@code &}, {@code ++}, {@code --},
	 * {@code sizeof}, or LHS operand of {@code =} or {@code .} operators.
	 * 
	 * @param inputType the pointer-generated type, or a simple type.
	 * @return type on which pointer-generation might have been applied.
	 */
	 ...
	 * {@code n e1 e2 e3...em...en} up to and excluding {@code em}, if
	 * <code>tillOpNode</code> is {@code em}, where all {@code e's} are postfix
	 * operators. Note that pointer-generation is performed on the returned type.
	 * 
	 * @param postFixExp
	 * @param tillOpNode
	 * @return
	...
	public static Type getHalfPostfixExpressionType(PostfixExpression n, APostfixOperation tillOpNode) {
		...
		if (e1Type == null) {
			if (operators.size() > 0
				...
				/*
				 * OLD CODE: The expression denoting a function call will belong to the same
				 * type as is the return type of the function being called. This old code below
				 * erroneously assumes the type of such function-calls to be function-type.
				 */
	...
	public Type visit(InitializerList n) {
		/*
		 * Note that pointer-generation doesn't matter here. This expression has a
		 * specific usage. ISO C doesn't discuss about the type of the initializer list.
		 */
	...
	public Type visit(CastExpressionTyped n) {
		Scopeable scope = Misc.getEnclosingBlock(n);
		if (scope == null) {
			scope = Program.getRoot();
		}
		return ExpressionTypeGetter.performPointerGeneration(Type.getTypeTree(n.getF1(), scope));
	}
	...
	public Type visit(Constant n) {
		...
		if (choiceNode instanceof NodeToken) {
			...
			if (choice.getKind() == CParserConstants.INTEGER_LITERAL) {
				...
				/*
				 * To handle "lu" and "ll" -- note that, at the time of writing this comment
				 * parser doesn't handle these suffixes anyway.
				 */
