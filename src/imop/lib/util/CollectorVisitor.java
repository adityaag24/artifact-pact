...
public class CollectorVisitor {
	...
	public static <T> Set<T> collectNodeSetInGenericGraph(Set<T> startPoints, Set<T> endPoints,
		...
		CallStack marker = new CallStack();
		CallStack.pushUnique(marker, CallStatement.getPhantomCall());
		outer: while (!workSet.isEmpty()) {
			T currentNode = Misc.getAnyElement(workSet);
			workSet.remove(currentNode);
			if (currentNode instanceof NodeWithStack) {
				Node actualNode = ((NodeWithStack) currentNode).getNode();
				if (actualNode instanceof PreCallNode) {
					CallStatement callStmt = ((PreCallNode) actualNode).getParent();
					if (callStmt.getInfo().isACallByFunctionPointer()) {
						Cell funcDesig = callStmt.getInfo().getFunctionDesignator();
						if (funcDesig != null) {
							if (funcDesig.getPointsTo(callStmt).isUniversal()
									|| funcDesig.getPointsTo(callStmt).isEmpty()) {
								Set<NodeWithStack> allNodes = new HashSet<>();
								for (FunctionDefinition functionDefinition : Program.getRoot().getInfo()
										.getAllFunctionDefinitions()) {
									if (functionDefinition == Program.getRoot().getInfo().getMainFunction()) {
										continue;
									}
									for (Node leafNode : functionDefinition.getInfo().getCFGInfo().getLexicalCFGLeafContents()) {
										allNodes.add(new NodeWithStack(leafNode, marker));
									}
								}
								/*
								 * NOTE: We conservatively assume that there are no endPoints.
								 */
								collectedNodeSet.addAll((Collection<? extends T>) allNodes);
								continue outer;
							}
						}
					}
