...
public class Normalization {
	...
	/**
	 * Given a node, this method traverses over all its constituent CFG leaf nodes
	 * in their postorder traversal, and transforms code such that each CFG leaf
	 * node respects the following constraints:
	 * <ol>
	 * <li>Each Declaration should contain declaration of only one variable.</li>
	 * <li>A typedef should not contain implicit declaration of any user-defined
	 * type.</li>
	 * <li>Declarations of user-defined types (structures, unions, and enumerators)
	 * should not also contain their use (i.e., declaration of their objects).</li>
	 * <li>The code should not contain any of the following operators : logical AND
	 * ( &&), logical OR ( ||), comma (,), and conditional (?:) operators.</li>
	 * <li>There should not be any chained assignment operators in the program (such
	 * as x = y = z;).</li>
	 * <li>No Expression should contain a function call; all function calls should
	 * be simplified such that they are denoted by independent statements
	...
	public static <T extends Node> T normalizeLeafNodes(T node, List<SideEffect> sideEffects) {
		...
		for (Node leaf : node.getInfo().getCFGInfo().getLexicalCFGLeafContentsInPostOrder()) {
			...
			/*
			 * Handles expression-statements separately, in case if they may have a
			 * function-call. Otherwise, replacement-string would contain a list of
			 * statements, which cannot be parsed as anything.
			 */
			if (leaf instanceof ExpressionStatement) {
				NodeOptional exp = ((ExpressionStatement) leaf).getF0();
				if (exp.accept(simplifier).hasPrelude() != 0) {
					CompoundStatement replacementBlock = FrontEnd.parseAndNormalize(
							"{" + simplifiedString.getReplacementString().toString() + "}", CompoundStatement.class);
					for (Node elem : replacementBlock.getInfo().getCFGInfo().getElementList()) {
						InsertImmediatePredecessor.insert(leaf, elem); // Handle these side effects at call
																		// sites of
																		// this method, if required.
					}
					NodeRemover.removeNode(leaf);
					continue;
				}
			}
	...
	/**
	 * Checks whether there exists any leaf CFG node within the given {@code node}
	 * which needs normalization of its expression or declarations.
	 * 
	 * @param node node to be checked for normalizations.
	 * @return <i>true</i> if there exists any leaf node in the given {@code node}
	 *         which needs normalization of expression or declaration; <i>false</i>
	 *         otherwise.
	 */
