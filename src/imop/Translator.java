...
public class Translator {
	...
	static String internalEnvironmentName = Builder.getNewTempName("internal");
	...
	void addCallStatement(CallStatement callStmt) {
		...
		if(calledDefinitions.isEmpty()) {
			return;
		}else if(functionName.startsWith("waitWrapper") || functionName.startsWith("signalWrapper")) {
			return;
	...
	void captureNamesAndDeclareMasterEnvironment(ForConstruct forConstruct) {
		...
		Builder.addDeclarationToGlobals(MUWOmpConverter.indexFunction(Program.getRoot().getInfo().getMainFunction()), 
...
//				+ "enqueueClosure(globalWorklist," + closureVarName + ");\n" + "}\n";
	...
	private ArrayList<String> processCallStatement(String functionName, ArrayList<String> chunk, Node element) {
		...
		if (calledDefinitions.isEmpty()) {
			...
			if (chunkFound) {
				...
				continuationFunction.getInfo().getCFGInfo().getBody().getInfo().removeExtraScopes();
				String continuationFunctionName = continuationFunction.getInfo().getFunctionName();
				if (functionCalledName.startsWith("waitWrapper") || functionCalledName.startsWith("signalWrapper")) {
	...
	private FunctionDefinition buildEnvironmentForFunction(String callerFunctionName, CallStatement callStatement,
		...
		if (!functionReturnTypeString.trim().equals("void")) {
			...
			Builder.addDeclarationToGlobals(
					MUWOmpConverter.indexFunction(Program.getRoot().getInfo().getMainFunction()),
		...
		for (Node element : elements) {
			if (element instanceof CallStatement) {
				...
				if (functionCalledName.startsWith("waitWrapper") 
						|| functionCalledName.startsWith("signalWrapper") 
						|| functionCalledName.startsWith("reduceWrapper")
	...
	public void translate() {
		...
		for (FunctionDefinition functionDefinition : Misc.getInheritedEnclosee(Program.getRoot(),
			...
			if (functionName.startsWith("waitWrapper") || functionName.startsWith("signalWrapper")) {
				continue;
			}
		...
		for(FunctionDefinition functionDefinition: Misc.getInheritedEnclosee(Program.getRoot(), FunctionDefinition.class)) {
			...
			for (ForConstruct forConstruct : Misc.getInheritedEnclosee(functionDefinition, ForConstruct.class)) {
				...
				ParallelConstruct enclosingParallelConstruct = MUWOmpConverter.getEnclosingNodeFromCalledPaths(forConstruct,
				...
				if (enclosingParallelConstruct != null) {
					ArrayList<String> privateAllocations = MUWOmpConverter.initializersMapper
...
//		addLocksToEnqueueOperations();
	...
	private void wrapForStatement(FunctionDefinition functionDefinition,ForStatement forStatement) {
		...
		if(shouldBeConverted) {
			...
			Builder.addDeclarationToGlobals(MUWOmpConverter.indexFunction(Program.getRoot().getInfo().getMainFunction()),functionSignatureNode);
			Builder.addFunctionToGlobal(functionDefinitionNode);
			functionDefinitionNode.getInfo().getCFGInfo().getBody().getInfo().removeExtraScopes();
	...
	private static ArrayList<String> renameStatements(CompoundStatement compoundStatement,
		...
		for (CallStatement callStatement : Misc.getInheritedEnclosee(compoundStatement, CallStatement.class)) {
			...
			if (functionsCalled.isEmpty()) {
				...
				if(functionCalledName.startsWith("waitWrapper") || functionCalledName.startsWith("signalWrapper")) {
					if(MUWOmpConverter.reductionVariableNameMapper.containsKey(functionCalledName)) {
						String reductionSymbolNewName = MUWOmpConverter.reductionVariableReducedMapper.get(functionCalledName);
						String reductionSymbolType = MUWOmpConverter.reductionVariableTypeMapper.get(functionCalledName);
		...
		for (Node elementStatement : elementsForStatements) {
			if (elementStatement instanceof Statement) {
				...
				Node renamedElement = MUWOmpConverter.myObtainRenamedNode(elementStatement, renameMapper);
	...
	public FunctionDefinition convertOmpForBodyToFunction(ForConstruct forConstruct) {
		...
		String loopVariable = MUWOmpConverter.forConstructMapper.get(forConstruct);
		...
		String nIterationsVarName = MUWOmpConverter.nIterationsVarName;
		String nThreadsVarDecl = "int "+nThreadsVarName+";";
		String nThreadsVarInit = nThreadsVarName + "=omp_get_num_threads();";
		for(UniqueForClauseSchedule schedule: Misc.getInheritedEnclosee(forConstruct,UniqueForClauseSchedule.class)) {
			ScheduleKind scheduleKind = schedule.getF2();
			NodeOptional chunkSizeOptional = schedule.getF3();
			if(scheduleKind.getF0().getChoice().toString().equals("static")) {
				if(chunkSizeOptional.present()) {

				}else {
					String condVarName = Builder.getNewTempName("cond");
					String initCond = "int "+condVarName+";";
					String condInit = condVarName + "=" +nIterationsVarName+">"+nThreadsVarName+";";
					String ifStatement = "if("+condVarName+"){K="+nIterationsVarName+"/"+nThreadsVarName+";}\nelse\n{K=1;}\n";
					chunkSize = "{"+initCond+"\n"+condInit+"\n"+ifStatement+"\n}\n";
				}
			}else {
				if(chunkSizeOptional.present()) {
		...
		String singleConstruct = "#pragma omp single\n";
		String singleConstructBody = "{\n" + nThreadsVarDecl + "\n" + nThreadsVarInit + "\n" + chunkSize + "\n}\n";
		CompoundStatement singleConstructBodyOptimized = FrontEnd.simplifyExpression(singleConstructBody);
		singleConstruct += singleConstructBodyOptimized.toString();
		String schedulerCode = "scheduler(readyWorklist,K);";
		...
		enclosingCompoundStatement.getInfo().getCFGInfo().addElement(indexForConstruct + 3,
				FrontEnd.parseAndNormalize(singleConstruct,SingleConstruct.class));
	...
	static FunctionDefinition convertStatementsToFunction(ForConstruct forConstruct, String functionPrefix,
		...
		String atomicDeclare = "#pragma omp critical";
		String enqueueClosure = "enqueueClosure(globalWorklist,"+closureVarName+");";
		String initPostbox = "initialisePostBox("+loopVariable+");";
		newCompoundStatement.add(masterEnvAlloc);
		newCompoundStatement.add(assignAllocStringFC);
		newCompoundStatement.add(initPostbox);
		newCompoundStatement.add(closureContinuationAssignment);
		newCompoundStatement.add(atomicDeclare);
