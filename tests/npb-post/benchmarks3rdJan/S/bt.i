...
struct WaitClsr{
    ...
    int start;
    int *waitIterations;
    void (*waitFunc)(struct WaitClsr *,struct Clsr *);
    void *redVar;
};
struct RedClsr{
    int n_reductions;
...
typedef struct WaitClsr WaitClosure;
typedef struct RedClsr ReductionClosure;
typedef struct Clsr Closure;
Closure *createClosure(TYPE_CLOSURE type,void (*fptr)(Closure *),void *env, Closure *continuation){
...
WorkListQueue *globalWorklist;
WorkListQueue *createWorklistQueue(){
    WorkListQueue *workListQueue = (WorkListQueue *)malloc(sizeof(WorkListQueue));
    workListQueue->front = ((void *)0);
    workListQueue->rear = ((void *)0);
    return workListQueue;
}
ClosureNode *createClosureNode(Closure *closure){
    ClosureNode *closureNode = (ClosureNode *)malloc(sizeof(ClosureNode));
    closureNode->closure = closure;
    closureNode->next = ((void *)0);
    return closureNode;
}
int isEmpty(WorkListQueue *workListQueue){
...
void enqueueClosure(WorkListQueue *queue,Closure *closure){
    ClosureNode *closureNode = createClosureNode(closure);
    if(isEmpty(queue)){
...
Closure *dequeueClosure(WorkListQueue *queue){
    if(isEmpty(queue)){
        return ((void *)0);
    }
    ClosureNode *closureNode = queue->front;
    Closure *closure = closureNode->closure;
...
void executeReadyWorkList(WorkListQueue *readyWorkList){
    while(!isEmpty(readyWorkList)){
        Closure *closure = dequeueClosure(readyWorkList);
        if(closure == ((void *)0)){
            return;
        }
        if(closure->typeClosure == NORMAL){
            closure->fptr(closure);
            if(closure->continuation!=((void *)0)){
                enqueueClosure(globalWorklist,closure->continuation);
            }
        }else if (closure->typeClosure==WAIT){
            WaitClosure *waitClsr = closure->waitRed.waitClsr;
            waitClsr->waitFunc(waitClsr,closure->continuation);
        }
    }
}
int isIdleThreads(int *threadStatus){
    if(threadStatus[omp_get_thread_num()]==1){
        return 0;
    }
    return 1;
}
void scheduler(WorkListQueue *readyWorkList,WorkListQueue *globalWorklist, atomic_flag *lock,int K,int *threadStatus){
    while(1){
        while(__atomic_test_and_set ((lock), 5));
        int isEmptyWL = isEmpty(globalWorklist);
        if(isEmptyWL){
        ...
        int cnt = K;
        while(cnt > 0){
            Closure *closure = dequeueClosure(globalWorklist);
            if(closure==((void *)0)){
                break;
            }
...
void executeWaitClosure(Closure *closure){
    WaitClosure *waitClosure = closure->waitRed.waitClsr;
    int currentIteration = waitClosure->currentIteration;
    int expr = waitClosure->expr;
    void (*redFunction)(void *,void *) = waitClosure->redFunction;
    int n_iters = waitClosure->n_iters;
    int start = waitClosure->start;
    int *waitIterations = waitClosure->waitIterations;
    void *redVar = waitClosure->redVar;
    mWait(currentIteration, closure,waitIterations, n_iters, redFunction,redVar, start);
}
WaitClosure *createWaitClosure(int currentIteration, int expr,void (*combine)(void *,void *),
                                             void *redVar, int start, int n_iters,
                                              int *waitIterations,Closure *newK){
    if(!expr){
            return 0;
    }
    WaitClosure *waitClosure = (WaitClosure *)malloc(sizeof(WaitClosure));
    waitClosure->currentIteration = currentIteration;
    waitClosure->expr = expr;
    waitClosure->redFunction = combine;
    waitClosure->redVar = redVar;
...
void mWait(int currentIteration,Closure *closure,
                    int *waitIterations, int n_iters,
                    void (*combine)(void *,void *),void *redVar, int start){
    int i;
    ...
    if(i!=n_iters){
        closure->waitRed.waitClsr->start = i;
...
typedef struct CommStruct Communicator;
typedef struct PostBoxStruct PostBox;
PostBox **p;
Message *createMessage(TYPE type,void *message){
...
QNode *createNode(TYPE type,void *message){
...
void printQueue(Queue *queue){
    printf("Printing from Queue %u\n",queue);
    if(queue == ((void *)0) || queue->rear==((void *)0) || queue->front==((void *)0)){
        printf("Queue Empty \n");
    }else{
        QNode *ptr = queue->front;
        while(ptr!=((void *)0)){
            printMessage(ptr->message);
            ptr = ptr->next;
        }
    }
}
void enQueue(Queue *queue, TYPE type,void *message){
    QNode *newNode = createNode(type,message);
    if(queue->rear==((void *)0)){
        queue->front=newNode;
        queue->rear=newNode;
        return;
    }
...
void incrementCounter(PostBox **p,int from,int to){
    (p[to]->communicators[from].counter)++;
}
void decrementCounter(PostBox **p,int from,int to){
    (p[to]->communicators[from].counter)--;
}
...
void postMessage(int from, int to, TYPE type,void *message){
    while(__atomic_test_and_set ((p[to]->communicators[from].lock), 5));
    incrementCounter(p,from,to);
    if(message == ((void *)0)){
        return;
    }
    enQueue(p[to]->communicators[from].messages,type,message);
    __atomic_clear ((p[to]->communicators[from].lock), 5);
}
void initialisePostBox(int n_p,int n_c){
   p = (PostBox **)malloc(n_p*sizeof(PostBox *));
   int i,j;
   for(i = 0;i < n_p;i++){
       p[i] = (PostBox *)malloc(sizeof(PostBox));
       p[i]->communicators = (Communicator *)malloc(n_c*sizeof(Communicator));
       for(j = 0;j < n_c;j++){
           p[i]->communicators[j].messages = createQueue();
           __atomic_clear ((p[i]->communicators[j].lock), 5);
       }
   }
}
...
void mSignal(int expr, int from,TYPE type,void *message,int n_iters,int *signalIterations){
    if(!expr){
        return;
    }
    int i;
    for(i = 0;i < n_iters;i++){
        int waitIter = signalIterations[i];
        postMessage(from,waitIter,type,message);
    }
}
...
#pragma omp master
 ...
 (3478.8*(double)n3-17655.7*((navg)*(navg))+28023.7*navg) / tmax;
  ...
  c_print_results("BT", class, grid_points[0],
    ...
    verified, "3.0 structured","31 Jan 2022", "gcc-11", "gcc-11", "(none)", "-I../common", "-fopenmp -O3 ",
    "-fopenmp ", "(none)");
}
static void add(void) {
  int i, j, k, m;
