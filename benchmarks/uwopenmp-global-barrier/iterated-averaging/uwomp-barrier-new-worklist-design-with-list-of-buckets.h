 ...
 * Each entry in the worklist is an array (bucket) of continuations.
 * The size of the array is MAX_ENTRIES_IN_BUCKET.
 */
typedef struct __worklist {
    /* This is an array of continuations. The size of this is
     * MAX_ENTRIES_IN_BUCKET.
     */
 ...
 * A bucket is an enrty in the worklist.
 */
int MAX_ENTRIES_IN_BUCKET = 0;

int *used_pending_wl_entry_index;
int *used_ready_wl_entry_index;
...
inline void uwomp_print_ready_worklist() {
  int tid = omp_get_thread_num();
	...
	fprintf(stderr, "------------------------------------\n", tid);
	while (p != NULL) {
		fprintf(stderr, "    %d: %x contn_env = %x", tid, p, p->contn_env);
		p = p -> next;
	}
	fprintf(stderr, "\n");
	return;
