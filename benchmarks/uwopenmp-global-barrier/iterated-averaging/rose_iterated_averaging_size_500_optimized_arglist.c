...
void parallel_for_body1_cps(void **arg_list,struct __cont_env *contn_env,int phase_num, struct __cont_env *old_env)
{
  ...
  // DONE: Arglist optmization
  // Do not free it now.
  //free(arg_list);
  // DONE: Ends

	//DONE: Reuse the memory allocated for previous environment
	...
	//}
	// DONE: Setting this continuation is not required since parallel_for_body1__while__0_cps
  ...
  //parallel_for_body1__while__0_cps(arg_list_1,parallel_for_body1_cps_cont_env1,phase_num);
	//DONE: 1. Pass the final argumetn as old_env.
	//      2. Change the second argument to contn_env - DONE
...
#pragma omp for  private(i)
  for (i = 1; i <= 500; i++) {
		// DONE: If continuation of the pfloop is empty we need not create this continuation
    ...
    // DONE: Arglist optimizaiton
    // Size change to 5 which is the maximum required in any funciton.
    //arg_list_2 = ((void **)(malloc(2 * sizeof(void *))));
    arg_list_2 = ((void **)(malloc(5 * sizeof(void *))));
    //num_mallocs++;
    // DONE: Ends
    arg_list_2[0] = i;
    arg_list_2[1] = iters;
...
void ompRegBody_cps_cont2(void **arg_list,struct __cont_env *contn_env,int phase_num, struct __cont_env *old_env)
{
  free(arg_list);
	...
	// DONE: Here ware not going reuse old_env.
	if (old_env != NULL) {
		free (old_env);
	}
...
void parallel_for_body1__while__0_cps(void **arg_list,struct __cont_env *contn_env,int phase_num, struct __cont_env *old_env)
{
  ...
  int i =  *p_i;
	if (!(delta > epsilon)) {
		...
		if (contn_env != NULL) {
			// TODO: Arglist optimization.
			// Should we free the arglist here or not???
      uwomp_call_contn_fun(contn_env);
    }
    // DONE: This is required to execute the the pending entries in the
    // ready worklist.
  ...
  diff[i] = fabs(newA[i] - oldA[i]);
    ...
    // DONE: Arglist optimizaiton
    // Reuse arglist
  //arg_list = ((void **)(realloc(arg_list, 5 * sizeof(void *))));
  // DONE: Ends
  arg_list[0] = __if_v0;
  arg_list[1] = i;
  arg_list[2] = iters;
...
void parallel_for_body1__while__0_cps_cont3(void **arg_list,struct __cont_env *contn_env,int phase_num, struct __cont_env *old_env)
{
  ...
  if (__if_v0) {
    ...
    // DONE: Arglist optimization Ends
    ...
    // DONE: Arglist optimization
    if (arg_list_3[i] == NULL) {
        arg_list_3[i] = ((void **)(malloc(2 * sizeof(void *))));
        //num_mallocs++;
    }
    // DONE: Arglist optimization Ends
    arg_list_3[i][0] = diff;
    arg_list_3[i][1] = &delta;
		// DONE:Add the final argument.
    //sum_cps(arg_list_3,parallel_for_body1__while__0_cps_cont_env4,phase_num);
    sum_cps(arg_list_3[i],parallel_for_body1__while__0_cps_cont_env4,phase_num, old_env);
  }
   else {
		 //DONE: Reuse the memory allocated for previous environment
		struct __cont_env *parallel_for_body1__while__0_cps_cont_env4;
...
void parallel_for_body1__while__0_cps_cont5(void **arg_list,struct __cont_env *contn_env,int phase_num, struct __cont_env *old_env)
{
   ...
   *p_iters = iters;
	//DONE: Reuse the memory allocated for previous environment
	//DONE: There is no need of creating this cotinuation.
