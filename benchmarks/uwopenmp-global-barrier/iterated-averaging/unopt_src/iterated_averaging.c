...
#pragma omp for
        for (j = 1; j <= N; j++) {
          if (__cani_enter(j,0, __gCnt) && (__Phase[j] == __cp && !__H[j][3])) {
            __H[j][3] = 1;
            __v0111[j] = __v01[j];
          }
        }
...
#pragma omp for
        for (j = 1; j <= N; j++) {
          if (__cani_enter(j,0, __gCnt) && (__Phase[j] == __cp && !__H[j][4])) {
            __H[j][4] = 1;
            if (__v0111[j]) {
              if (j == 1) {
                delta = sum(diff);
...
#pragma omp for 
        for (j = 1; j <= N; j++) {
          if (__cani_enter(j,0, __gCnt) && (__Phase[j] == __cp && !__H[j][5])) {
            __H[j][5] = 1;
            if (__v0111[j]) 
              __Phase[j]++;
          }
...
#pragma omp for
        for (j = 1; j <= N; j++) {
          if (__cani_enter(j,0, __gCnt) && (__Phase[j] == __cp && !__H[j][6])) {
            __H[j][6] = 1;
            if (__v0111[j]) {
              __reset(j,0,0);
            }
...
#pragma omp for 
        for (j = 1; j <= N; j++) {
          if (__cani_enter(j,0, __gCnt) && (__Phase[j] == __cp && !__H[j][7])) {
            __H[j][7] = 1;
            if (!(__vWhile00[j] != 0)) {
#pragma omp critical 
              __count0++;
