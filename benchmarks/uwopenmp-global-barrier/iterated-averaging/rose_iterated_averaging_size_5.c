...
// initialize the prng with a fixed seed.
  srand48(101);
  oldA = ((double *)(malloc(sizeof(double ) * (5 + 2))));
  newA = ((double *)(malloc(sizeof(double ) * (5 + 2))));
  diff = ((double *)(malloc(sizeof(double ) * (5 + 2))));
...
void parallel_for_body1_cps(void **arg_list,struct __cont_env *contn_env,int phase_num, struct __cont_env *old_env)
{
  ...
  parallel_for_body1__while__0_cps(arg_list_1,contn_env,phase_num, old_env);
}
