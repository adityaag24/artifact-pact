...
void parallel_for_body1__while__0(int c,int i,int j);
void parallel_for_body1(int c,int i,int j);
void OmpRegBody1();
int size_x;
int size_y;
...
#pragma omp single 
{
      uwomp_worklist_init((omp_get_num_threads()),32563);
    }
...
#pragma omp single 
{
    ...
    arg_list_OmpRegBody1_0 = calloc(size_x - -1,sizeof(void **));
    arg_list_parallel_for_body1__while__0_0 = calloc(size_x - -1,sizeof(void **));
  }
...
#pragma omp for 
  for (i = 0; i <= size_x; i++) {
    ...
    arg_list_parallel_for_body1_0[i][0] = c;
    arg_list_parallel_for_body1_0[i][1] = i;
    arg_list_parallel_for_body1_0[i][2] = j;
