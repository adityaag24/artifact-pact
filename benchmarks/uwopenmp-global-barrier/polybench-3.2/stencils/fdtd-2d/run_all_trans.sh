...
size=4000
threads=( 16 )
echo "SIZE   THREADS AvgRuntime" >> fdtd_trans.results
...
do
    ...
    do
        ...
        rtime=`cat fdtd_trans.out | head -n 1 | awk '{print $1}'`
        echo $i $rtime
    ...
    echo "Average Running Time for fdtd_trans$size+$thread = " $avg
    echo "$size $thread $avg" >> fdtd_trans.results
done
