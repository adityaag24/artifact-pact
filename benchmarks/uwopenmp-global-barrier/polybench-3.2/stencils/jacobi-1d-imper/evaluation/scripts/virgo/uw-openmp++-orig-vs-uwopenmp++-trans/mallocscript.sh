...
cp $SRCDIR/unopt_src/jacobi-1d-imper-malloc.c $SRCDIR/rose_jacobi-1d-imper_final.c
echo -n -e "uwopenmp\t$SIZE\t$TSTEPS\t$NTASKS\t$NTHREADS\t$NRUNS\t$MACHINE " >> results.txt
./mallocrun_all_trans.sh $MACHINE $NRUNS $SIZE $TSTEPS $NTHREADS >> results.txt
...
cp $SRCDIR/rose_jacobi-1d-imper-no-reuseopt-malloc.c $SRCDIR/rose_jacobi-1d-imper_final.c
...
cp $SRCDIR/rose_jacobi-1d-imper-reuse-opt-malloc.c $SRCDIR/rose_jacobi-1d-imper_final.c
...
cp $SRCDIR/rose_jacobi-1d-imper-opt-malloc.c $SRCDIR/rose_jacobi-1d-imper_final.c
