...
#pragma omp single 
{
    ...
    __parreg_arglist = ((void **)(malloc(10 * sizeof(void *))));
...
#pragma omp critical
    ...
    __parreg_arglist[3] = &t;
    __parreg_arglist[4] = &tsteps;
...
/* Stop and print timer. */
  ...
  ;
	fprintf(stdout, "Number of mallocs = %d\n", numMallocs);
...
#pragma omp for  private(i,j)
  for (i = 1; i <  *n - 1; i++) {
    if (arg_list_parallel_for_body1_0[i] == NULL) {
			arg_list_parallel_for_body1_0[i] = malloc(10 * sizeof(void *));

			//fprintf(stderr, "%u\n", arg_list_parallel_for_body1_0[i]);
#pragma omp critical
			{
				numMallocs++;
			}
    ...
    arg_list_parallel_for_body1_0[i][3] = t;
    arg_list_parallel_for_body1_0[i][4] = tsteps;
    struct __cont_env *OmpRegBody1_cps_cps_cont_env_1_1 = malloc(sizeof(struct __cont_env ));

		//fprintf(stderr, "%u\n", OmpRegBody1_cps_cps_cont_env_1_1);
...
//   else {
    parallel_for_body1__while__0_cps_cont_env3 = malloc(sizeof(struct __cont_env ));
		...
		//fprintf(stderr, "%u\n", parallel_for_body1__while__0_cps_cont_env3);
#pragma omp critical
		{
			numMallocs++;
		}
...
//   else {
    parallel_for_body1__while__0_cps_cont_env4 = malloc(sizeof(struct __cont_env ));
		...
		//fprintf(stderr, "%u\n", parallel_for_body1__while__0_cps_cont_env4);
#pragma omp critical
		{
			numMallocs++;
		}
