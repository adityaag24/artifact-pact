...
machine=$1
num_of_runs=$2
echo "SIZE  NTHREADS AvgRuntime" >> gemver_trans.results
for size in $(seq 8000 8000)
do
    echo $size
    threads=$size
    ...
    echo "Average Running Time for gemver_trans$size+$size = " $avg
    echo "$size   $threads   $avg" >> gemver_trans.results
done
