...
rm trans; gcc -O3 -fopenmp -I $SRCDIR/../../../utilities -I $SRCDIR $SRCDIR/../../../utilities/polybench.c -DPOLYBENCH_TIME $SRCDIR/rose_gemver_final.c -o trans


rundir="/home/induk/raghesh/uw-openmp/benchmarks/uwopenmp-global-barrier/polybench-3.2/linear-algebra/kernels/gemver/evaluation/scripts/k2/uw-openmp++-orig-vs-uwopenmp++-trans"


echo "#!/bin/bash
#PBS -l mem=4gb,nodes=$MACHINE:ppn=$NTHREADS,walltime=10:00:00
#PBS -m abe
#PBS -o gemver_trans.out
#PBS -e gemver_trans.err
...
do
	...
	rtime=`cat gemver_trans.out | grep "Run time" | awk '{print $4}'`
	echo $i $rtime 1>&2
