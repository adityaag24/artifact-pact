...
void init() {
    ...
    void *_imopVarPre113;
    _imopVarPre112 = sizeof(double *);
    _imopVarPre113 = calloc(2048, _imopVarPre112);
    u = (double **) _imopVarPre113;
    unsigned long int _imopVarPre116;
    void *_imopVarPre117;
    _imopVarPre116 = sizeof(double *);
    _imopVarPre117 = calloc(2048, _imopVarPre116);
    w = (double **) _imopVarPre117;
    unsigned long int _imopVarPre120;
    void *_imopVarPre121;
    _imopVarPre120 = sizeof(double);
    _imopVarPre121 = calloc(2048, _imopVarPre120);
    delta = (double *) _imopVarPre121;
    for (i = 0; i < 2048; i++) {
        unsigned long int _imopVarPre124;
        void *_imopVarPre125;
        _imopVarPre124 = sizeof(double);
        _imopVarPre125 = calloc(2048, _imopVarPre124);
        u[i] = (double *) _imopVarPre125;
        unsigned long int _imopVarPre128;
        void *_imopVarPre129;
        _imopVarPre128 = sizeof(double);
        _imopVarPre129 = calloc(2048, _imopVarPre128);
        w[i] = (double *) _imopVarPre129;
    }
    for (i = 1; i < 2048 - 1; i++) {
        w[i][0] = 100.0;
        w[i][2048 - 1] = 100.0;
    ...
    mean = 0.0;
    for (i = 1; i < 2048 - 1; i++) {
        mean = mean + w[i][0] + w[i][2048 - 1];
    }
    for (j = 0; j < 2048; j++) {
        mean = mean + w[2048 - 1][j] + w[0][j];
    }
    mean = mean / (double) (2 * 2048 + 2 * 2048 - 4);
    for (i = 1; i < 2048 - 1; i++) {
        for (j = 1; j < 2048 - 1; j++) {
            w[i][j] = mean;
            u[i][j] = mean;
...
struct ENV_SHARED_ivp7 {
    double diff;
    int k;
} ;
struct ENV_PRIVATE_ivp9 {
    int i;
    int j;
} ;
struct master_env {
    struct internal_tf_ivp35 *internal_vf_ivp36;
    struct internal_t_ivp38 *internal_v_ivp37;
    struct internal_t_ivp40 *internal_v_ivp39;
} ;
struct internal_tf_ivp35 {
    void ( *continuation )(struct Clsr *);
    struct ENV_SHARED_ivp7 *env_s_ivp8;
    struct ENV_PRIVATE_ivp9 *envp_c_ivp3;
} ;
void ompfor_ivp41(struct Clsr *clsr_v_ivp1);
struct internal_t_ivp38 {
    void ( *continuation )(struct Clsr *);
    struct ENV_PRIVATE_ivp9 *envp_c_ivp3;
    struct ENV_SHARED_ivp7 *env_s_ivp8;
} ;
void while_ivp32_ivp70(struct ENV_PRIVATE_ivp9 *envp_c_ivp3, struct ENV_SHARED_ivp7 *env_s_ivp8 , struct Clsr *clsr_v_ivp1);
struct internal_t_ivp40 {
    void ( *continuation )(struct Clsr *);
    struct ENV_SHARED_ivp7 *env_s_ivp8;
    struct ENV_PRIVATE_ivp9 *envp_c_ivp3;
} ;
void fun_ivp33_ivp73(struct ENV_SHARED_ivp7 *env_s_ivp8, struct ENV_PRIVATE_ivp9 *envp_c_ivp3 , struct Clsr *clsr_v_ivp1);
void clsr_ivp83(struct Clsr *clsr_v_ivp1);
void clsr_ivp84(struct Clsr *clsr_v_ivp1);
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        int _imopVarPre132;
        _imopVarPre131 = &epsilon;
        _imopVarPre132 = scanf("%f", _imopVarPre131);
        success = _imopVarPre132;
    } else {
        float *_imopVarPre135;
        char *_imopVarPre136;
        int _imopVarPre137;
        _imopVarPre135 = &epsilon;
        _imopVarPre136 = argv[1];
        _imopVarPre137 = sscanf(_imopVarPre136, "%f", _imopVarPre135);
        success = _imopVarPre137;
    }
    if (success != 1) {
        printf("\n");
        printf("HEATED_PLATE\n");
    ...
    struct ENV_SHARED_ivp7 *env_s_ivp8;
    void *env_s_ivp13;
    int tempSize_ivp14 = sizeof(struct ENV_SHARED_ivp7);
    env_s_ivp13 = malloc(tempSize_ivp14);
    env_s_ivp8 = (struct ENV_SHARED_ivp7 *) env_s_ivp13;
    env_s_ivp8->k = k;
    env_s_ivp8->diff = diff;
#pragma omp parallel private(i, j) shared(u, w)
    {
        int _imopVarPre69;
        struct ENV_PRIVATE_ivp9 *env_p_ivp10;
        void *env_p_ivp15;
        int tempSize_ivp16 = sizeof(struct ENV_PRIVATE_ivp9);
        env_p_ivp15 = malloc(tempSize_ivp16);
        env_p_ivp10 = (struct ENV_PRIVATE_ivp9 *) env_p_ivp15;
        i = (env_p_ivp10->i);
#pragma omp single nowait
        {
            startRange_ivp17 = 1;
            endRange_ivp17 = 2048 - 1 - 1;
            ...
            leftPointer = (int *) _imopVarPre22;
            unsigned long int _imopVarPre25;
            void *_imopVarPre26;
            _imopVarPre25 = nThreads_ivp17 * sizeof(int);
            _imopVarPre26 = malloc(_imopVarPre25);
            rightPointer = (int *) _imopVarPre26;
            unsigned long int _imopVarPre29;
            void *_imopVarPre30;
            _imopVarPre29 = (nIters_ivp4 + startRange_ivp17) * sizeof(struct Clsr *);
            _imopVarPre30 = malloc(_imopVarPre29);
            readyWorkList = (struct Clsr **) _imopVarPre30;
        }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
        _imopVarPre69 = omp_get_thread_num();
        int tid_ivp17 = _imopVarPre69;
...
#pragma omp for nowait
        for (i = 1; i < 2048 - 1; i++) {
            struct ENV_PRIVATE_ivp9 *envp_c_ivp3;
            void *envp_t_ivp11;
            int tempSize_ivp12 = sizeof(struct ENV_PRIVATE_ivp9);
            envp_t_ivp11 = malloc(tempSize_ivp12);
            envp_c_ivp3 = (struct ENV_PRIVATE_ivp9 *) envp_t_ivp11;
            envp_c_ivp3->j = (env_p_ivp10->j);
            envp_c_ivp3->i = (env_p_ivp10->i);
            envp_c_ivp3->i = i;
            void ( *_imopVarPre45 )(struct Clsr *clsr_v_ivp1);
            struct Clsr *_imopVarPre46;
            _imopVarPre45 = &ompfor_ivp41;
            _imopVarPre46 = createClosure(NORMAL, _imopVarPre45, i);
            struct Clsr *clsr_v_ivp1 = _imopVarPre46;
            unsigned long int _imopVarPre49;
            void *_imopVarPre50;
            _imopVarPre49 = sizeof(struct master_env);
            _imopVarPre50 = malloc(_imopVarPre49);
            clsr_v_ivp1->environment = (struct master_env *) _imopVarPre50;
            unsigned long int _imopVarPre53;
            void *_imopVarPre54;
            _imopVarPre53 = sizeof(struct internal_tf_ivp35);
            _imopVarPre54 = malloc(_imopVarPre53);
            clsr_v_ivp1->environment->internal_vf_ivp36 = (struct internal_tf_ivp35 *) _imopVarPre54;
            clsr_v_ivp1->environment->internal_vf_ivp36->continuation = &lastFunction;
            int _imopVarPre56;
            _imopVarPre56 = omp_get_thread_num();
            if (i < leftPointer[_imopVarPre56]) {
                int _imopVarPre57;
                _imopVarPre57 = omp_get_thread_num();
                leftPointer[_imopVarPre57] = i;
            }
            int _imopVarPre59;
            _imopVarPre59 = omp_get_thread_num();
            if (i > rightPointer[_imopVarPre59]) {
                int _imopVarPre60;
                _imopVarPre60 = omp_get_thread_num();
                rightPointer[_imopVarPre60] = i;
            }
            readyWorkList[i] = clsr_v_ivp1;
            unsigned long int _imopVarPre63;
            void *_imopVarPre64;
            _imopVarPre63 = sizeof(struct internal_t_ivp38);
            _imopVarPre64 = malloc(_imopVarPre63);
            clsr_v_ivp1->environment->internal_v_ivp37 = (struct internal_t_ivp38 *) _imopVarPre64;
            unsigned long int _imopVarPre67;
            void *_imopVarPre68;
            _imopVarPre67 = sizeof(struct internal_t_ivp40);
            _imopVarPre68 = malloc(_imopVarPre67);
            clsr_v_ivp1->environment->internal_v_ivp39 = (struct internal_t_ivp40 *) _imopVarPre68;
            clsr_v_ivp1->environment->internal_vf_ivp36->env_s_ivp8 = env_s_ivp8;
            clsr_v_ivp1->environment->internal_vf_ivp36->envp_c_ivp3 = envp_c_ivp3;
        }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
...
void ompfor_ivp41(struct Clsr *clsr_v_ivp1) {
    struct ENV_PRIVATE_ivp9 *envp_c_ivp3_ivp42;
    struct ENV_SHARED_ivp7 *env_s_ivp8_ivp43;
    env_s_ivp8_ivp43 = clsr_v_ivp1->environment->internal_vf_ivp36->env_s_ivp8;
    envp_c_ivp3_ivp42 = clsr_v_ivp1->environment->internal_vf_ivp36->envp_c_ivp3;
    clsr_v_ivp1->environment->internal_v_ivp37->continuation = clsr_v_ivp1->environment->internal_vf_ivp36->continuation;
    while_ivp32_ivp70(envp_c_ivp3_ivp42, env_s_ivp8_ivp43, clsr_v_ivp1);
}
void while_ivp32_ivp70(struct ENV_PRIVATE_ivp9 *envp_c_ivp3, struct ENV_SHARED_ivp7 *env_s_ivp8 , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp37->envp_c_ivp3 = envp_c_ivp3;
    clsr_v_ivp1->environment->internal_v_ivp37->env_s_ivp8 = env_s_ivp8;
    if (epsilon <= (clsr_v_ivp1->environment->internal_v_ivp37->env_s_ivp8->diff)) {
        struct ENV_PRIVATE_ivp9 *envp_c_ivp3_ivp71;
        struct ENV_SHARED_ivp7 *env_s_ivp8_ivp72;
        env_s_ivp8_ivp72 = clsr_v_ivp1->environment->internal_v_ivp37->env_s_ivp8;
        envp_c_ivp3_ivp71 = clsr_v_ivp1->environment->internal_v_ivp37->envp_c_ivp3;
        clsr_v_ivp1->environment->internal_v_ivp39->continuation = clsr_v_ivp1->environment->internal_v_ivp37->continuation;
        fun_ivp33_ivp73(env_s_ivp8_ivp72, envp_c_ivp3_ivp71, clsr_v_ivp1);
    } else {
        clsr_v_ivp1->fptr = clsr_v_ivp1->environment->internal_v_ivp37->continuation;
        void ( *_imopVarPre80 )(struct Clsr *);
        _imopVarPre80 = clsr_v_ivp1->fptr;
        _imopVarPre80(clsr_v_ivp1);
    }
}
void fun_ivp33_ivp73(struct ENV_SHARED_ivp7 *env_s_ivp8, struct ENV_PRIVATE_ivp9 *envp_c_ivp3 , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8 = env_s_ivp8;
    clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3 = envp_c_ivp3;
    delta[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] = 0.0;
    for ((clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j) = 1; (clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j) < 2048 - 1; (clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)++) {
        w[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)] = (u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i) - 1][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)] + u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i) + 1][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)] + u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j) - 1] + u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j) + 1]) / 4.0;
        if (w[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)] > u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)]) {
            delta[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] = delta[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] + (w[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)] - u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)]);
        } else {
            delta[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] = delta[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] + (u[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)] - w[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)][(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->j)]);
        }
    }
    clsr_v_ivp1->fptr = &clsr_ivp84;
    mWaitAll(clsr_v_ivp1);
}
void clsr_ivp83(struct Clsr *clsr_v_ivp1) {
    struct ENV_PRIVATE_ivp9 *envp_c_ivp3_ivp76;
    struct ENV_SHARED_ivp7 *env_s_ivp8_ivp77;
    env_s_ivp8_ivp77 = clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8;
    envp_c_ivp3_ivp76 = clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3;
    clsr_v_ivp1->environment->internal_v_ivp37->continuation = clsr_v_ivp1->environment->internal_v_ivp39->continuation;
    while_ivp32_ivp70(envp_c_ivp3_ivp76, env_s_ivp8_ivp77, clsr_v_ivp1);
}
void clsr_ivp84(struct Clsr *clsr_v_ivp1) {
    if ((clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i) == 1) {
        for ((clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k) = 1; (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k) < 2048 - 1; (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k)++) {
            (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->diff) += delta[(clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k)];
        }
        (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->diff) = (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->diff) / (double) (2048 - 1) / (double) (2048 - 1);
        iterations++;
        temp = u;
        u = w;
        w = temp;
    }
    clsr_v_ivp1->fptr = &clsr_ivp83;
    mWaitAll(clsr_v_ivp1);
}
