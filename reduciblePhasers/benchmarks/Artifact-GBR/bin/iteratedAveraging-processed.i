...
void executeReadyWorkList() {
    int _imopVarPre105;
    _imopVarPre105 = omp_get_thread_num();
    int tid = _imopVarPre105;
    ...
    while (1) {
        ...
        for (i = leftPtr; i <= rightPtr; i++) {
            ...
            if (closure->typeClosure == NORMAL) {
                void ( *_imopVarPre107 )(struct Clsr *);
                _imopVarPre107 = closure->fptr;
                _imopVarPre107(closure);
            } else {
                if (closure->typeClosure == WAIT) {
                    struct WaitClsr *waitClosure = closure->waitClosure;
                    void ( *_imopVarPre109 )(struct Clsr *);
                    _imopVarPre109 = waitClosure->waitFunction;
                    _imopVarPre109(closure);
                }
            }
...
void mWaitAll(struct Clsr *closure) {
    int _imopVarPre110;
    _imopVarPre110 = omp_get_thread_num();
    int tid = _imopVarPre110;
...
void while_ivp32_ivp70(struct ENV_SHARED_ivp7 *env_s_ivp8, struct ENV_PRIVATE_ivp9 *envp_c_ivp3 , struct Clsr *clsr_v_ivp1);
...
int main() {
    ...
    srand48(101);
    int _imopVarPre114;
    unsigned long int _imopVarPre115;
    void *_imopVarPre116;
    _imopVarPre114 = (4096 + 2);
    _imopVarPre115 = sizeof(double);
    _imopVarPre116 = calloc(_imopVarPre115, _imopVarPre114);
    oldA = (double *) _imopVarPre116;
    int _imopVarPre120;
    unsigned long int _imopVarPre121;
    void *_imopVarPre122;
    _imopVarPre120 = (4096 + 2);
    _imopVarPre121 = sizeof(double);
    _imopVarPre122 = calloc(_imopVarPre121, _imopVarPre120);
    newA = (double *) _imopVarPre122;
    int _imopVarPre126;
    unsigned long int _imopVarPre127;
    void *_imopVarPre128;
    _imopVarPre126 = (4096 + 2);
    _imopVarPre127 = sizeof(double);
    _imopVarPre128 = calloc(_imopVarPre127, _imopVarPre126);
    diff = (double *) _imopVarPre128;
    fprintf(stdout, "======Iterated Averaging======\n");
    oldA[0] = 0;
    for (j = 1; j <= 4096; j++) {
        double _imopVarPre129;
        _imopVarPre129 = drand48();
        oldA[j] = _imopVarPre129;
    }
    oldA[4096 + 1] = 0;
    epsilon = 0.006;
    delta = epsilon;
...
void ompfor_ivp41(struct Clsr *clsr_v_ivp1) {
    ...
    while_ivp32_ivp70(env_s_ivp8_ivp43, envp_c_ivp3_ivp42, clsr_v_ivp1);
}
void while_ivp32_ivp70(struct ENV_SHARED_ivp7 *env_s_ivp8, struct ENV_PRIVATE_ivp9 *envp_c_ivp3 , struct Clsr *clsr_v_ivp1) {
...
void fun_ivp33_ivp73(struct ENV_SHARED_ivp7 *env_s_ivp8, struct ENV_PRIVATE_ivp9 *envp_c_ivp3 , struct Clsr *clsr_v_ivp1) {
    ...
    newA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] = (oldA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i) - 1] + oldA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i) + 1]) / 2.0;
    if (newA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] > oldA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)]) {
        diff[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] = newA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] - oldA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)];
    } else {
        diff[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] = oldA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)] - newA[(clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i)];
    }
...
void clsr_ivp83(struct Clsr *clsr_v_ivp1) {
    ...
    while_ivp32_ivp70(env_s_ivp8_ivp77, envp_c_ivp3_ivp76, clsr_v_ivp1);
}
void clsr_ivp84(struct Clsr *clsr_v_ivp1) {
    if ((clsr_v_ivp1->environment->internal_v_ivp39->envp_c_ivp3->i) == 1) {
        for ((clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k) = 1; (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k) <= 4096; (clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k)++) {
            delta += diff[(clsr_v_ivp1->environment->internal_v_ivp39->env_s_ivp8->k)];
        }
        delta = delta / 4096;
        iters++;
        temp = newA;
