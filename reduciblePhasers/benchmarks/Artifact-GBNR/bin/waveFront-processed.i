...
void init() {
    ...
    void *_imopVarPre112;
    _imopVarPre111 = sizeof(float *) * 16384;
    ...
    for (i = 0; i < 16384; i++) {
        ...
        void *_imopVarPre115;
        _imopVarPre114 = sizeof(float) * 16384;
        _imopVarPre115 = malloc(_imopVarPre114);
        A[i] = _imopVarPre115;
    }
    for (i = 0; i < 16384; i++) {
        for (j = 0; j < 16384; j++) {
            signed long int _imopVarPre117;
            _imopVarPre117 = random();
            A[i][j] = (float) _imopVarPre117 / 2147483647;
        }
    }
    return;
}
...
void recTask_ivp64(int i, int t , struct Clsr *clsr_v_ivp1);
...
void fun_ivp28_ivp67(int t, int i , struct Clsr *clsr_v_ivp1);
...
void ompfor_ivp36(struct Clsr *clsr_v_ivp1) {
    ...
    recTask_ivp64(i_ivp14_ivp37, 1, clsr_v_ivp1);
}
void recTask_ivp64(int i, int t , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp32->i = i;
    clsr_v_ivp1->environment->internal_v_ivp32->t = t;
    if (clsr_v_ivp1->environment->internal_v_ivp32->t <= (16384 + 16384 - 3)) {
        int t_ivp65;
        int i_ivp66;
        t_ivp65 = clsr_v_ivp1->environment->internal_v_ivp32->t;
        ...
        fun_ivp28_ivp67(t_ivp65, i_ivp66, clsr_v_ivp1);
    } else {
...
void fun_ivp28_ivp67(int t, int i , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp34->t = t;
    clsr_v_ivp1->environment->internal_v_ivp34->i = i;
    clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre121 = clsr_v_ivp1->environment->internal_v_ivp34->t >= clsr_v_ivp1->environment->internal_v_ivp34->i;
    if (clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre121) {
        clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre121 = clsr_v_ivp1->environment->internal_v_ivp34->t < 16384 + clsr_v_ivp1->environment->internal_v_ivp34->i - 1;
    }
    if (clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre121) {
        clsr_v_ivp1->environment->internal_v_ivp34->b = clsr_v_ivp1->environment->internal_v_ivp34->i;
        clsr_v_ivp1->environment->internal_v_ivp34->a = clsr_v_ivp1->environment->internal_v_ivp34->t - clsr_v_ivp1->environment->internal_v_ivp34->i + 1;
        A[clsr_v_ivp1->environment->internal_v_ivp34->a][clsr_v_ivp1->environment->internal_v_ivp34->b] = A[clsr_v_ivp1->environment->internal_v_ivp34->a - 1][clsr_v_ivp1->environment->internal_v_ivp34->b] + A[clsr_v_ivp1->environment->internal_v_ivp34->a][clsr_v_ivp1->environment->internal_v_ivp34->b - 1];
    }
    clsr_v_ivp1->fptr = &clsr_ivp76;
    mWaitAll(clsr_v_ivp1);
}
void clsr_ivp76(struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre123 = clsr_v_ivp1->environment->internal_v_ivp34->t + 1;
    int i_ivp69;
    int _imopVarPre123_ivp70;
    i_ivp69 = clsr_v_ivp1->environment->internal_v_ivp34->i;
    _imopVarPre123_ivp70 = clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre123;
    clsr_v_ivp1->environment->internal_v_ivp32->continuation = clsr_v_ivp1->environment->internal_v_ivp34->continuation;
    recTask_ivp64(i_ivp69, _imopVarPre123_ivp70, clsr_v_ivp1);
}
