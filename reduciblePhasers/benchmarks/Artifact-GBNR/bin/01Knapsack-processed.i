...
void knapSack_ivp64(int i, int k , struct Clsr *clsr_v_ivp1);
...
void fun_ivp28_ivp67(int i, int k , struct Clsr *clsr_v_ivp1);
...
int main(int argc, char **argv) {
    ...
    if (argc > 2) {
        char *_imopVarPre118;
        int _imopVarPre119;
        _imopVarPre118 = argv[2];
        _imopVarPre119 = atoi(_imopVarPre118);
        verbose = _imopVarPre119;
    } else {
        verbose = 0;
    }
    int *_imopVarPre122;
    ...
    _imopVarPre122 = &C;
    _imopVarPre123 = &N;
    fscanf(fp, "%d %d", _imopVarPre123, _imopVarPre122);
    printf("The number of objects is %d, and the capacity is %d.\n", N, C);
    size = N * sizeof(int);
    void *_imopVarPre125;
    _imopVarPre125 = malloc(size);
    weights = (int *) _imopVarPre125;
    void *_imopVarPre127;
    _imopVarPre127 = malloc(size);
    profits = (int *) _imopVarPre127;
    void *_imopVarPre129;
    _imopVarPre129 = malloc(size);
    temp = (int *) _imopVarPre129;
    int _imopVarPre130;
    _imopVarPre130 = weights == ((void *) 0);
    if (!_imopVarPre130) {
        _imopVarPre130 = profits == ((void *) 0);
    }
    if (_imopVarPre130) {
    ...
    for (k = 0; k < N; k++) {
        ...
        int *_imopVarPre134;
        int _imopVarPre135;
        _imopVarPre133 = &(profits[k]);
        _imopVarPre134 = &(weights[k]);
        _imopVarPre135 = fscanf(fp, "%d %d", _imopVarPre134, _imopVarPre133);
        count = _imopVarPre135;
        if (count != 2) {
...
#pragma omp barrier
    ...
    double _imopVarPre137;
    _imopVarPre137 = (ftime - itime);
...
void ompfor_ivp36(struct Clsr *clsr_v_ivp1) {
    ...
    knapSack_ivp64(0, i_ivp14_ivp37, clsr_v_ivp1);
}
void knapSack_ivp64(int i, int k , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp32->i = i;
    clsr_v_ivp1->environment->internal_v_ivp32->k = k;
    if (clsr_v_ivp1->environment->internal_v_ivp32->i < N) {
        int i_ivp65;
        int k_ivp66;
        i_ivp65 = clsr_v_ivp1->environment->internal_v_ivp32->i;
        k_ivp66 = clsr_v_ivp1->environment->internal_v_ivp32->k;
        clsr_v_ivp1->environment->internal_v_ivp34->continuation = clsr_v_ivp1->environment->internal_v_ivp32->continuation;
        fun_ivp28_ivp67(i_ivp65, k_ivp66, clsr_v_ivp1);
    } else {
...
void fun_ivp28_ivp67(int i, int k , struct Clsr *clsr_v_ivp1) {
    ...
    clsr_v_ivp1->environment->internal_v_ivp34->k = k;
    if (clsr_v_ivp1->environment->internal_v_ivp34->k == 0) {
        newProfit[clsr_v_ivp1->environment->internal_v_ivp34->k] = 0;
    }
    if (clsr_v_ivp1->environment->internal_v_ivp34->k < weights[clsr_v_ivp1->environment->internal_v_ivp34->i]) {
        newProfit[clsr_v_ivp1->environment->internal_v_ivp34->k] = oldProfit[clsr_v_ivp1->environment->internal_v_ivp34->k];
    } else {
        int _imopVarPre140;
        int _imopVarPre141;
        _imopVarPre140 = (oldProfit[clsr_v_ivp1->environment->internal_v_ivp34->k]) > (profits[clsr_v_ivp1->environment->internal_v_ivp34->i] + oldProfit[clsr_v_ivp1->environment->internal_v_ivp34->k - weights[clsr_v_ivp1->environment->internal_v_ivp34->i]]);
        if (_imopVarPre140) {
            _imopVarPre141 = (oldProfit[clsr_v_ivp1->environment->internal_v_ivp34->k]);
        } else {
            _imopVarPre141 = (profits[clsr_v_ivp1->environment->internal_v_ivp34->i] + oldProfit[clsr_v_ivp1->environment->internal_v_ivp34->k - weights[clsr_v_ivp1->environment->internal_v_ivp34->i]]);
        }
        newProfit[clsr_v_ivp1->environment->internal_v_ivp34->k] = _imopVarPre141;
    }
...
void clsr_ivp76(struct Clsr *clsr_v_ivp1) {
    ...
    newProfit = oldProfit;
    oldProfit = temp;
    clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre143 = clsr_v_ivp1->environment->internal_v_ivp34->i + 1;
    int _imopVarPre143_ivp69;
    int k_ivp70;
    _imopVarPre143_ivp69 = clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre143;
    k_ivp70 = clsr_v_ivp1->environment->internal_v_ivp34->k;
    clsr_v_ivp1->environment->internal_v_ivp32->continuation = clsr_v_ivp1->environment->internal_v_ivp34->continuation;
    knapSack_ivp64(_imopVarPre143_ivp69, k_ivp70, clsr_v_ivp1);
}
