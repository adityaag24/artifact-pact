...
void printTable() {
    ...
    for (i = 0; i <= size_x; i++) {
        for (j = 0; j <= size_y; j++) {
            int _imopVarPre111;
            _imopVarPre111 = table[i][j];
            fprintf(stdout, "%d ", _imopVarPre111);
        }
        fprintf(stdout, "\n");
    }
}
...
void longestSubSeq_ivp64(int i, int c , struct Clsr *clsr_v_ivp1);
...
void fun_ivp28_ivp67(int c, int i , struct Clsr *clsr_v_ivp1);
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        char *_imopVarPre113;
        struct _IO_FILE *_imopVarPre114;
        _imopVarPre113 = argv[1];
        _imopVarPre114 = fopen(_imopVarPre113, "r");
        fp = _imopVarPre114;
        if (fp == ((void *) 0)) {
            char *_imopVarPre116;
        ...
        char *_imopVarPre118;
        _imopVarPre118 = argv[0];
        printf("USAGE : %s [filename].\n", _imopVarPre118);
        exit(1);
    }
    int *_imopVarPre121;
    int *_imopVarPre122;
    _imopVarPre121 = &size_y;
    _imopVarPre122 = &size_x;
    fscanf(fp, "%d %d ", _imopVarPre122, _imopVarPre121);
    unsigned long int _imopVarPre124;
    void *_imopVarPre125;
    _imopVarPre124 = size_x * sizeof(char);
    ...
    void *_imopVarPre128;
    _imopVarPre127 = size_x * sizeof(char);
    ...
    int _imopVarPre133;
    void *_imopVarPre134;
    _imopVarPre132 = sizeof(int *);
    _imopVarPre133 = size_x + 1;
    _imopVarPre134 = calloc(_imopVarPre133, _imopVarPre132);
    table = (int **) _imopVarPre134;
    for (i = 0; i < size_x + 1; i++) {
        unsigned long int _imopVarPre138;
        int _imopVarPre139;
        void *_imopVarPre140;
        _imopVarPre138 = sizeof(int);
        _imopVarPre139 = size_y + 1;
        _imopVarPre140 = calloc(_imopVarPre139, _imopVarPre138);
        table[i] = (int *) _imopVarPre140;
    }
    itime = omp_get_wtime();
#pragma omp parallel
    {
...
#pragma omp barrier
    ...
    _imopVarPre142 = (ftime - itime);
    fprintf(stderr, "Runtime = %lf seconds.\n", _imopVarPre142);
    int _imopVarPre144;
    _imopVarPre144 = table[size_x][size_y];
...
void ompfor_ivp36(struct Clsr *clsr_v_ivp1) {
    ...
    longestSubSeq_ivp64(i_ivp14_ivp37, 0, clsr_v_ivp1);
}
void longestSubSeq_ivp64(int i, int c , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp32->i = i;
    clsr_v_ivp1->environment->internal_v_ivp32->c = c;
    if (clsr_v_ivp1->environment->internal_v_ivp32->c <= size_x + size_y) {
        int c_ivp65;
        int i_ivp66;
        c_ivp65 = clsr_v_ivp1->environment->internal_v_ivp32->c;
        i_ivp66 = clsr_v_ivp1->environment->internal_v_ivp32->i;
        clsr_v_ivp1->environment->internal_v_ivp34->continuation = clsr_v_ivp1->environment->internal_v_ivp32->continuation;
        fun_ivp28_ivp67(c_ivp65, i_ivp66, clsr_v_ivp1);
    } else {
...
void fun_ivp28_ivp67(int c, int i , struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp34->c = c;
    clsr_v_ivp1->environment->internal_v_ivp34->i = i;
    clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre146 = clsr_v_ivp1->environment->internal_v_ivp34->c >= clsr_v_ivp1->environment->internal_v_ivp34->i;
    if (clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre146) {
        clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre146 = clsr_v_ivp1->environment->internal_v_ivp34->c <= clsr_v_ivp1->environment->internal_v_ivp34->i + size_y;
    }
    if (clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre146) {
        clsr_v_ivp1->environment->internal_v_ivp34->j = clsr_v_ivp1->environment->internal_v_ivp34->c - clsr_v_ivp1->environment->internal_v_ivp34->i;
        int _imopVarPre147;
        _imopVarPre147 = clsr_v_ivp1->environment->internal_v_ivp34->i == 0;
        if (!_imopVarPre147) {
            _imopVarPre147 = clsr_v_ivp1->environment->internal_v_ivp34->j == 0;
        }
        if (_imopVarPre147) {
            table[clsr_v_ivp1->environment->internal_v_ivp34->i][clsr_v_ivp1->environment->internal_v_ivp34->j] = 0;
        } else {
            if (x[clsr_v_ivp1->environment->internal_v_ivp34->i - 1] == y[clsr_v_ivp1->environment->internal_v_ivp34->j - 1]) {
                table[clsr_v_ivp1->environment->internal_v_ivp34->i][clsr_v_ivp1->environment->internal_v_ivp34->j] = 1 + table[clsr_v_ivp1->environment->internal_v_ivp34->i - 1][clsr_v_ivp1->environment->internal_v_ivp34->j - 1];
            } else {
                int _imopVarPre150;
                int _imopVarPre151;
                _imopVarPre150 = (table[clsr_v_ivp1->environment->internal_v_ivp34->i - 1][clsr_v_ivp1->environment->internal_v_ivp34->j]) > (table[clsr_v_ivp1->environment->internal_v_ivp34->i][clsr_v_ivp1->environment->internal_v_ivp34->j - 1]);
                if (_imopVarPre150) {
                    _imopVarPre151 = (table[clsr_v_ivp1->environment->internal_v_ivp34->i - 1][clsr_v_ivp1->environment->internal_v_ivp34->j]);
                } else {
                    _imopVarPre151 = (table[clsr_v_ivp1->environment->internal_v_ivp34->i][clsr_v_ivp1->environment->internal_v_ivp34->j - 1]);
                }
                table[clsr_v_ivp1->environment->internal_v_ivp34->i][clsr_v_ivp1->environment->internal_v_ivp34->j] = _imopVarPre151;
            }
        }
    }
...
void clsr_ivp76(struct Clsr *clsr_v_ivp1) {
    clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre153 = clsr_v_ivp1->environment->internal_v_ivp34->c + 1;
    int _imopVarPre153_ivp69;
    int i_ivp70;
    _imopVarPre153_ivp69 = clsr_v_ivp1->environment->internal_v_ivp34->_imopVarPre153;
    i_ivp70 = clsr_v_ivp1->environment->internal_v_ivp34->i;
    clsr_v_ivp1->environment->internal_v_ivp32->continuation = clsr_v_ivp1->environment->internal_v_ivp34->continuation;
    longestSubSeq_ivp64(i_ivp70, _imopVarPre153_ivp69, clsr_v_ivp1);
}
