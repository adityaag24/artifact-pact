...
void init() {
    ...
    int i;
    for (i = 0; i < 16384; i++) {
        A[i] = ((double) (i + 2)) / 16384;
        B[i] = ((double) (i + 3)) / 16384;
    }
}
...
struct master_env {
    struct internal_tf_ivp63 *internal_vf_ivp64;
    struct internal_t_ivp66 *internal_v_ivp65;
...
void ompfor_ivp69(struct Clsr *clsr_v_ivp12);
struct internal_t_ivp66 {
    void ( *continuation )(struct Clsr *);
    struct ENV_PRIVATE_ivp39 *envp_c_ivp14;
} ;
void while_ivp60_ivp119(struct ENV_PRIVATE_ivp39 *envp_c_ivp14, struct Clsr *clsr_v_ivp12);
...
void fun_ivp61_ivp121(struct ENV_PRIVATE_ivp39 *envp_c_ivp14, struct Clsr *clsr_v_ivp12);
void clsr_ivp136(struct Clsr *clsr_v_ivp12);
void clsr_ivp137(struct Clsr *clsr_v_ivp12);
void clsr_ivp138(struct Clsr *clsr_v_ivp12);
void clsr_ivp139(struct Clsr *clsr_v_ivp12);
...
#pragma omp parallel private(i, j)
    {
        struct WLQueue *_imopVarPre58;
        _imopVarPre58 = createWorklistQueue();
        struct WLQueue *readyWorklist = _imopVarPre58;
#pragma omp single nowait
        {
            int _imopVarPre57;
            globalWorklist = createWorklistQueue();
            _imopVarPre57 = omp_get_num_threads();
            int nThreads_ivp16 = _imopVarPre57;
        }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
        struct ENV_PRIVATE_ivp39 *env_p_ivp40;
        void *env_p_ivp43;
        int tempSize_ivp44 = sizeof(struct ENV_PRIVATE_ivp39);
        env_p_ivp43 = malloc(tempSize_ivp44);
        env_p_ivp40 = (struct ENV_PRIVATE_ivp39 *) env_p_ivp43;
        int t = 0;
        env_p_ivp40->t = 0;
        i = (env_p_ivp40->i);
#pragma omp single nowait
        {
            ...
            unsigned long int _imopVarPre55;
            void *_imopVarPre56;
            _imopVarPre55 = (nIters_ivp15 + startRange_ivp17) * sizeof(struct HashTable *);
            _imopVarPre56 = malloc(_imopVarPre55);
            postbox_ivp17 = (struct HashTable **) _imopVarPre56;
        }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
#pragma omp for private(i) nowait
        for (i = 1; i < 16384 - 1; i++) {
            ...
            envp_c_ivp14->i = i;
            struct Clsr *_imopVarPre84;
            void ( *_imopVarPre83 )(struct Clsr *clsr_v_ivp12);
            _imopVarPre83 = &ompfor_ivp69;
            _imopVarPre84 = createClosure(NORMAL, _imopVarPre83);
            struct Clsr *clsr_v_ivp12 = _imopVarPre84;
            unsigned long int _imopVarPre87;
            void *_imopVarPre88;
            _imopVarPre87 = sizeof(struct master_env);
            _imopVarPre88 = malloc(_imopVarPre87);
            clsr_v_ivp12->environment = (struct master_env *) _imopVarPre88;
            unsigned long int _imopVarPre95;
            void *_imopVarPre96;
            _imopVarPre95 = sizeof(struct internal_tf_ivp63);
            _imopVarPre96 = malloc(_imopVarPre95);
            clsr_v_ivp12->environment->internal_vf_ivp64 = (struct internal_tf_ivp63 *) _imopVarPre96;
            initialisePostBox(i);
            clsr_v_ivp12->environment->internal_vf_ivp64->continuation = &lastFunction;
// #pragma omp dummyFlush CRITICAL_START
#pragma omp critical
            {
...
// #pragma omp dummyFlush CRITICAL_END
            ...
            void *_imopVarPre104;
            _imopVarPre103 = sizeof(struct internal_t_ivp68);
            _imopVarPre104 = malloc(_imopVarPre103);
            clsr_v_ivp12->environment->internal_v_ivp67 = (struct internal_t_ivp68 *) _imopVarPre104;
            unsigned long int _imopVarPre111;
            void *_imopVarPre112;
            _imopVarPre111 = sizeof(struct internal_t_ivp66);
            _imopVarPre112 = malloc(_imopVarPre111);
            clsr_v_ivp12->environment->internal_v_ivp65 = (struct internal_t_ivp66 *) _imopVarPre112;
            clsr_v_ivp12->environment->internal_vf_ivp64->envp_c_ivp14 = envp_c_ivp14;
        }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
#pragma omp single nowait
        {
            int nThreads_ivp117;
            nThreads_ivp117 = omp_get_num_threads();
            int cond_ivp118;
            cond_ivp118 = nIters_ivp15 > nThreads_ivp117;
            if (cond_ivp118) {
                K = nIters_ivp15 / nThreads_ivp117;
            } else {
                K = 1;
            }
        }
...
void waitWrapper_ivp17(int i, struct Clsr *clsr_v_ivp12) {
    ...
    if (1) {
        ...
        unsigned long int _imopVarPre221;
        void *_imopVarPre222;
        _imopVarPre221 = n_iters * sizeof(int);
        _imopVarPre222 = malloc(_imopVarPre221);
        int *q = (int *) _imopVarPre222;
        int i_ivp = 0;
...
void signalWrapper_ivp22(int i, struct Clsr *clsr_v_ivp12) {
    ...
    if (1) {
        int p[] = {i - 1, i + 1};
        int n_iters = sizeof (p) / sizeof (p[0]);
        unsigned long int _imopVarPre217;
        void *_imopVarPre218;
        _imopVarPre217 = n_iters * sizeof(int);
        _imopVarPre218 = malloc(_imopVarPre217);
        int *q_ivp = (int *) _imopVarPre218;
        int i_ivp = 0;
        for (i_ivp = 0; i_ivp < n_iters; i_ivp++) {
            q_ivp[i_ivp] = p[i_ivp];
        }
...
void signalWrapper_ivp27(int i, struct Clsr *clsr_v_ivp12) {
    ...
    if (1) {
        int p[] = {i - 1, i + 1};
        int n_iters = sizeof (p) / sizeof (p[0]);
        unsigned long int _imopVarPre209;
        void *_imopVarPre210;
        _imopVarPre209 = n_iters * sizeof(int);
        _imopVarPre210 = malloc(_imopVarPre209);
        int *q_ivp = (int *) _imopVarPre210;
        int i_ivp = 0;
        for (i_ivp = 0; i_ivp < n_iters; i_ivp++) {
            q_ivp[i_ivp] = p[i_ivp];
        }
...
void waitWrapper_ivp32(int i, struct Clsr *clsr_v_ivp12) {
    ...
    if (1) {
        ...
        void *_imopVarPre214;
        _imopVarPre213 = n_iters * sizeof(int);
        _imopVarPre214 = malloc(_imopVarPre213);
        int *q = (int *) _imopVarPre214;
        int i_ivp = 0;
        for (i_ivp = 0; i_ivp < n_iters; i_ivp++) {
            q[i_ivp] = p[i_ivp];
        }
...
void ompfor_ivp69(struct Clsr *clsr_v_ivp12) {
    struct ENV_PRIVATE_ivp39 *envp_c_ivp14_ivp70;
    envp_c_ivp14_ivp70 = clsr_v_ivp12->environment->internal_vf_ivp64->envp_c_ivp14;
    clsr_v_ivp12->environment->internal_v_ivp65->continuation = clsr_v_ivp12->environment->internal_vf_ivp64->continuation;
    while_ivp60_ivp119(envp_c_ivp14_ivp70, clsr_v_ivp12);
}
void while_ivp60_ivp119(struct ENV_PRIVATE_ivp39 *envp_c_ivp14, struct Clsr *clsr_v_ivp12) {
    clsr_v_ivp12->environment->internal_v_ivp65->envp_c_ivp14 = envp_c_ivp14;
    if ((clsr_v_ivp12->environment->internal_v_ivp65->envp_c_ivp14->t) < 50) {
        struct ENV_PRIVATE_ivp39 *envp_c_ivp14_ivp120;
        envp_c_ivp14_ivp120 = clsr_v_ivp12->environment->internal_v_ivp65->envp_c_ivp14;
        clsr_v_ivp12->environment->internal_v_ivp67->continuation = clsr_v_ivp12->environment->internal_v_ivp65->continuation;
        fun_ivp61_ivp121(envp_c_ivp14_ivp120, clsr_v_ivp12);
    } else {
        clsr_v_ivp12->fptr = clsr_v_ivp12->environment->internal_v_ivp65->continuation;
        void ( *_imopVarPre133 )(struct Clsr *);
        _imopVarPre133 = clsr_v_ivp12->fptr;
        _imopVarPre133(clsr_v_ivp12);
    }
}
void fun_ivp61_ivp121(struct ENV_PRIVATE_ivp39 *envp_c_ivp14, struct Clsr *clsr_v_ivp12) {
    clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14 = envp_c_ivp14;
    B[(clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i)] = 0.3333 * (A[(clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i) - 1] + A[(clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i)] + A[(clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i) + 1]);
    clsr_v_ivp12->environment->internal_v_ivp67->i_ivp45 = (clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i);
    int i_ivp45_ivp122;
    i_ivp45_ivp122 = clsr_v_ivp12->environment->internal_v_ivp67->i_ivp45;
    clsr_v_ivp12->fptr = &clsr_ivp139;
    signalWrapper_ivp27(i_ivp45_ivp122, clsr_v_ivp12);
}
void clsr_ivp136(struct Clsr *clsr_v_ivp12) {
    (clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->t)++;
    struct ENV_PRIVATE_ivp39 *envp_c_ivp14_ivp130;
    envp_c_ivp14_ivp130 = clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14;
    clsr_v_ivp12->environment->internal_v_ivp65->continuation = clsr_v_ivp12->environment->internal_v_ivp67->continuation;
    while_ivp60_ivp119(envp_c_ivp14_ivp130, clsr_v_ivp12);
}
void clsr_ivp137(struct Clsr *clsr_v_ivp12) {
    clsr_v_ivp12->environment->internal_v_ivp67->i_ivp51 = (clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i);
    int i_ivp51_ivp128;
    i_ivp51_ivp128 = clsr_v_ivp12->environment->internal_v_ivp67->i_ivp51;
    clsr_v_ivp12->fptr = &clsr_ivp136;
    waitWrapper_ivp17(i_ivp51_ivp128, clsr_v_ivp12);
}
void clsr_ivp138(struct Clsr *clsr_v_ivp12) {
    A[(clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i)] = B[(clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i)];
    clsr_v_ivp12->environment->internal_v_ivp67->i_ivp49 = (clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i);
    int i_ivp49_ivp126;
    i_ivp49_ivp126 = clsr_v_ivp12->environment->internal_v_ivp67->i_ivp49;
    clsr_v_ivp12->fptr = &clsr_ivp137;
    signalWrapper_ivp22(i_ivp49_ivp126, clsr_v_ivp12);
}
void clsr_ivp139(struct Clsr *clsr_v_ivp12) {
    clsr_v_ivp12->environment->internal_v_ivp67->i_ivp47 = (clsr_v_ivp12->environment->internal_v_ivp67->envp_c_ivp14->i);
    int i_ivp47_ivp124;
    i_ivp47_ivp124 = clsr_v_ivp12->environment->internal_v_ivp67->i_ivp47;
    clsr_v_ivp12->fptr = &clsr_ivp138;
    waitWrapper_ivp32(i_ivp47_ivp124, clsr_v_ivp12);
}
